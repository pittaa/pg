...
class Programator(Zamestnanec):
    def vypocitej_mzdu(self):
        # Zavoláme implementaci z rodičovské třídy (super)
        zakladni_vypocet = super().vypocitej_mzdu()
        # Přidáme 10 % navíc (vracím jako int pro hezčí výpis bez desetinných míst)
        return int(zakladni_vypocet * 1.10)
...
class Manazer(Zamestnanec):
    def __init__(self, jmeno, zakladni_mzda, pocet_podrizenych):
        # Zavoláme konstruktor rodiče pro nastavení jména a mzdy
        super().__init__(jmeno, zakladni_mzda)
        self.pocet_podrizenych = pocet_podrizenych

    def vypocitej_mzdu(self):
        # Zavoláme implementaci z rodičovské třídy
        zakladni_vypocet = super().vypocitej_mzdu()
        # Přičteme bonus za podřízené
